...
if [ "$1" = "service-catalog" ]; then
   MINISHIFT_ENABLE_EXPERIMENTAL=y minishift start --vm-driver generic --remote-ipaddress 127.0.0.1 --remote-ssh-user `whoami` --remote-ssh-key ~/.ssh/id_minishift_rsa --extra-clusterup-flags "--enable=*,service-catalog,automation-service-broker" 
else
